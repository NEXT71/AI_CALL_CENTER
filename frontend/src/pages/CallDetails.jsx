        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          {call.qualityMetrics && (
            <div className="card-enhanced p-6">
              ...
              {/* AI Quality Factors - New Section */}
              {call.qualityMetrics.aiFactors && (
                <div className="mb-6 p-4 bg-gradient-to-br from-blue-50 to-purple-50 rounded-xl border border-blue-200">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between p-3 bg-white/80 backdrop-blur rounded-lg">
                      <span className="text-sm font-medium text-slate-700">Customer Tone</span>
                      <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                        call.qualityMetrics.aiDetails?.customer_tone === 'positive' ? 'bg-green-100 text-green-700' :
                        call.qualityMetrics.aiDetails?.customer_tone === 'angry' || 
                        call.qualityMetrics.aiDetails?.customer_tone === 'frustrated' ? 'bg-red-100 text-red-700' :
                        'bg-blue-100 text-blue-700'
                      }`}>
                        {call.qualityMetrics.aiDetails?.customer_tone || 'N/A'}
                      </span>
                    </div>
                    <div className="flex items-center justify-between p-3 bg-white/80 backdrop-blur rounded-lg">
                      <span className="text-sm font-medium text-slate-700">Agent Professionalism</span>
                      <span className="px-3 py-1 rounded-lg bg-blue-100 text-blue-900 text-xs font-bold">
                        {call.qualityMetrics.aiFactors?.agent_professionalism_score || 0}/25
                      </span>
                    </div>
                    <div className="flex items-center justify-between p-3 bg-white/80 backdrop-blur rounded-lg">
                      <span className="text-sm font-medium text-slate-700">Customer Communication</span>
                      <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                        call.qualityMetrics.aiDetails?.customer_style === 'polite' ? 'bg-green-100 text-green-700' :
                        call.qualityMetrics.aiDetails?.customer_style === 'aggressive' ? 'bg-red-100 text-red-700' :
                        'bg-blue-100 text-blue-700'
                      }`}>
                        {call.qualityMetrics.aiDetails?.customer_style || 'N/A'}
                      </span>
                    </div>
                    
                    {/* Critical Flags */}
                    {call.qualityMetrics.aiFlags && (
                      <div className="mt-3 space-y-2">
                        {call.qualityMetrics.aiFlags.is_dnc_customer && (
                          <div className="flex items-center gap-2 p-3 bg-red-50 border border-red-200 rounded-lg">
                            <AlertTriangle size={16} className="text-red-600 flex-shrink-0" />
                            <span className="text-sm font-bold text-red-900">DNC Customer - Do Not Call Again</span>
                          </div>
                        )}
                        {call.qualityMetrics.aiFlags.has_abusive_language && (
                          <div className="flex items-center gap-2 p-3 bg-red-50 border border-red-200 rounded-lg">
                            <AlertTriangle size={16} className="text-red-600 flex-shrink-0" />
                            <span className="text-sm font-bold text-red-900">Abusive Language Detected</span>
                          </div>
                        )}
                        {call.qualityMetrics.aiFlags.agent_too_casual && (
                          <div className="flex items-center gap-2 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                            <AlertTriangle size={16} className="text-amber-600 flex-shrink-0" />
                            <span className="text-sm font-bold text-amber-900">Agent Used Casual Language</span>
                          </div>
                        )}
                        {call.qualityMetrics.aiFlags.customer_frustrated && (
                          <div className="flex items-center gap-2 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                            ...
                            <span className="text-sm font-bold text-orange-900">Customer Showed Frustration</span>
                          </div>
                        )}
                      </div>
                    )}
